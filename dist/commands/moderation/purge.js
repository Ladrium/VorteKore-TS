...
class default_1 extends Command_1.Command {
    ...
    run(message, args) {
        ...
        if (!args[0])
            return message.channel.send(new lib_1.VorteEmbed(message).baseEmbed().setDescription("Please provide a number."));
        ...
        if (member) {
            ...
            message.channel.messages.fetch({
                ...
                message.channel.send(new lib_1.VorteEmbed(message).baseEmbed().setDescription(`Successfully deleted ${num} messages.`));
            });
        }
        else {
